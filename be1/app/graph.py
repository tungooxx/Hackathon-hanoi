...
def _customer_visible_slots(slots: dict[str, Any], schema: list[Any] | None = None) -> dict[str, Any]:
    ...
    if "budget_min" in visible:
        visible["ngân sách từ"] = visible.pop("budget_min")
    if "budget_target" in visible:
        visible["ngân sách khoảng"] = visible.pop("budget_target")
    return visible
...
async def budget_clarify_node(state: AgentState) -> dict:
    w = get_stream_writer()
    amount = state.get("budget_pending")
    display = f"{amount / 1_000_000:g} triệu" if amount and amount >= 1_000_000 else f"{amount:,.0f}đ"
    message = (
        f"Dạ {display} là mức mình muốn chi tối đa, muốn tìm từ mức đó trở lên, "
        "hay chỉ cần giá quanh mức đó ạ?"
    )
    w({"type": "text_chunk", "content": message})
    w({"type": "done", "turn_type": "budget_clarify"})
    return _assistant_history(state, message)
